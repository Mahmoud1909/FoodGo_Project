                ...
                <div class="col-12">
                    <div class="resttab-sec">
                        <div class="menu-tab">
                            <ul>
                                <li class="active">
                                    <a href="{{ route('restaurants.control.edit', $id) }}">Restaurant Details</a>
                                </li>
                            </ul>
                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        window.waitForFirestore(function(db) {
            ...
            console.log('✅ [CONTROL EDIT] Firestore ready, initializing page...');
        ...
        function initRestaurantEditPage() {
            ...
            console.log('🔄 [CONTROL EDIT] Starting data loading process...');
            console.log('🔄 [CONTROL EDIT] Restaurant ID:', id);
            var dbStatus = database ? 'Available' : 'NOT AVAILABLE';
            console.log('🔄 [CONTROL EDIT] Database instance:', dbStatus);
            var refStatus = ref ? 'Available' : 'NOT AVAILABLE';
            console.log('🔄 [CONTROL EDIT] Reference object:', refStatus);
            var refPath = ref ? ref.path : 'N/A';
            console.log('🔄 [CONTROL EDIT] Reference path:', refPath);
            console.log('🔄 [CONTROL EDIT] ============================================');
            
            // Validate prerequisites
            if (!database) {
                console.error('❌ [CONTROL EDIT] CRITICAL: Database is not initialized!');
            ...
            if (!ref) {
                console.error('❌ [CONTROL EDIT] CRITICAL: Reference is not initialized!');
            ...
            if (!id || id === '') {
                console.error('❌ [CONTROL EDIT] CRITICAL: Restaurant ID is empty!');
            ...
            console.log('🔄 [CONTROL EDIT] Attempting to fetch document from Firestore...');
            updateStatusMessage('Fetching restaurant data from Firestore collection: vendors | Document ID: ' + id, 'info');
            console.log('🔄 [CONTROL EDIT] Collection: vendors');
            console.log('🔄 [CONTROL EDIT] Document ID:', id);
            updateStatusMessage('Fetching restaurant data from Firestore collection: vendors | Document ID: ' + id, 'info');
            
            ref.get().then(async function(docSnapshot) {
                console.log('🔄 [CONTROL EDIT] ============================================');
                console.log('🔄 [CONTROL EDIT] Firestore query completed');
                var snapshotStatus = docSnapshot ? 'Yes' : 'No';
                console.log('🔄 [CONTROL EDIT] Document snapshot received:', snapshotStatus);
                ...
                try {
                    ...
                    console.log('✅ [CONTROL EDIT] Document exists: YES');
                    console.log('✅ [CONTROL EDIT] Document ID:', docSnapshot.id);
                    console.log('🔄 [CONTROL EDIT] Extracting document data...');
                    
                    updateStatusMessage('Extracting restaurant data from document...', 'info');
                    
                    var restaurant = docSnapshot.data();
                    if (!restaurant) {
                        console.error('❌ [CONTROL EDIT] CRITICAL: Document data is null!');
                    ...
                    console.log('✅ [CONTROL EDIT] Restaurant data extracted successfully!');
                    ...
                    // Profile route removed for control edit page
                ...
                if (restaurant.hasOwnProperty('isSelfDelivery') && restaurant.isSelfDelivery != null && restaurant.isSelfDelivery != '' && restaurant.isSelfDelivery) {
                    ...
                    console.log('✅ [CONTROL EDIT] All data loaded and displayed successfully!');
                ...
                console.error('❌ [CONTROL EDIT] CRITICAL: Firestore query failed!');
            ...
            $(".edit-form-btn").click(async function() {
                ...
                if (restaurantname == '') {
                    ...
                    await storeStoryData().then(async (resStoryVid) => {
                        ...
                        await storeImageData().then(async (IMG) => {
                            ...
                            await storeGalleryImageData().then(async (GalleryIMG) => {
                                ...
                                await storeMenuImageData().then(async (MenuIMG) => {
                                    updateStatusMessage('All images processed. Saving restaurant data to Firestore...', 'info');
                                    geoFirestore.collection('vendors').doc(id).update({
                                        ...
                                        if (resStoryVid.length > 0 || story_thumbnail != '') {
                                            if (resStoryVid.length > 0 && story_thumbnail == '') {
                                                jQuery("#data-table_processing").hide();
                                                updateStatusMessage('Error: Story thumbnail is required when video is provided.', 'error');
                                                $(".error_top").show();
                                                $(".error_top").html("");
                                                ...
                                                jQuery("#data-table_processing").hide();
                                                updateStatusMessage('Error: Story video is required when thumbnail is provided.', 'error');
                                                $(".error_top").show();
                                                $(".error_top").html("");
                                                $(".error_top").append("<p>{{ trans('lang.story_error') }}</p>");
                                                window.scrollTo(0, 0);
                                            } else {
                                                updateStatusMessage('Saving story data to Firestore...', 'info');
                                        ...
                                        updateStatusMessage('Error saving restaurant data: ' + (error.message || 'Unknown error'), 'error');
                                        jQuery("#data-table_processing").hide();
                                        $(".error_top").show();
                                        $(".error_top").html("");
                                        $(".error_top").append("<p>Error: " + (error.message || 'Unknown error') + "</p>");
                                        window.scrollTo(0, 0);
                                    });
                                }).catch(err => {
                                    updateStatusMessage('Error saving menu images: ' + (err.message || err), 'error');
                                    jQuery("#data-table_processing").hide();
                                    $(".error_top").show();
                                    $(".error_top").html("");
                                ...
                                updateStatusMessage('Error saving gallery images: ' + (err.message || err), 'error');
                                jQuery("#data-table_processing").hide();
                                $(".error_top").show();
                                $(".error_top").html("");
                                $(".error_top").append("<p>" + err + "</p>");
                            ...
                            updateStatusMessage('Error saving story images: ' + (err.message || err), 'error');
                            jQuery("#data-table_processing").hide();
                            $(".error_top").show();
                            $(".error_top").html("");
                            $(".error_top").append("<p>" + err + "</p>");
                        ...
                        updateStatusMessage('Error saving story video: ' + (err.message || err), 'error');
                        jQuery("#data-table_processing").hide();
                        $(".error_top").show();
                        $(".error_top").html("");
                        $(".error_top").append("<p>" + err + "</p>");
