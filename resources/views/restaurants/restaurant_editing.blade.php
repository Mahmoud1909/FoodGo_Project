                ...
                <div class="col-12">
                    <div class="resttab-sec">
                        <div class="menu-tab">
                            <ul>
                                <li class="active">
                                    <a href="{{ route('restaurants.view', $id) }}">Profile</a>
                                </li>
                                <li class="active">
                                    <a href="{{ route('restaurants.control.editing', $id) }}">Restaurant</a>
                                </li>
                            </ul>
                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        var storyImagesRef = null;
        var isSelfDelivery = false;
        var selfDeliveryRef = null;
        ...
        window.waitForFirestore(function(db) {
            ...
            console.log('🔄 [RESTAURANT EDITING] ============================================');
            console.log('🔄 [RESTAURANT EDITING] Initializing Firestore references...');
            console.log('🔄 [RESTAURANT EDITING] Restaurant ID:', id);
            console.log('🔄 [RESTAURANT EDITING] Database available:', !!database);
            
            if (!id || id === '' || id === undefined) {
                console.error('❌ [RESTAURANT EDITING] CRITICAL: Restaurant ID is empty or invalid!');
                console.error('❌ [RESTAURANT EDITING] Cannot create Firestore reference without ID');
                updateStatusMessage('Error: Restaurant ID is missing. Please check the URL.', 'error');
            ...
            console.log('✅ [RESTAURANT EDITING] Vendors reference created');
            console.log('✅ [RESTAURANT EDITING] Reference path:', ref.path);
            console.log('✅ [RESTAURANT EDITING] Reference ID:', ref.id);
            console.log('✅ [RESTAURANT EDITING] All Firestore references initialized');
            console.log('🔄 [RESTAURANT EDITING] ============================================');
            
            // Verify ref is properly created
            if (!ref) {
                console.error('❌ [RESTAURANT EDITING] CRITICAL: Failed to create Firestore reference!');
                updateStatusMessage('Error: Failed to create Firestore reference. Please refresh the page.', 'error');
                toastr.error('Failed to create Firestore reference. Please refresh the page.');
                return;
            }
            ...
            // Load self delivery settings
            selfDeliveryRef.get().then(async function(settingSnapshots) {
                if (settingSnapshots.exists && settingSnapshots.data()) {
                ...
                console.warn('Error loading self delivery settings:', error);
            });
            ...
            database.collection('zone').where('publish', '==', true).get().then(async function(snapshots) {
                ...
                // Sort zones by name in JavaScript
                zones.sort((a, b) => {
            ...
            ref_deliverycharge.get().then(async function(snapshots_charge) {
                if (snapshots_charge.exists) {
                    ...
                    try {
                            ...
                            ref_charge.get().then(async function(snapshots) {
                                if (snapshots.docs.length > 0) {
                                    try {
                                        ...
                                        console.warn('Error loading delivery charge:', error);
                                    }
                                }
                            }).catch(function(error) {
                                console.warn('Error loading vendor delivery charge:', error);
                            });
                        } else {
                        ...
                        console.warn('Error processing delivery charge settings:', error);
                    }
                }
            }).catch(function(error) {
                console.warn('Error loading delivery charge settings:', error);
            });
            ...
            database.collection('settings').doc("story").get().then(async function(snapshots) {
                ...
                console.warn('Error loading story settings:', error);
            });
            
            // Load special discount offer settings
            database.collection('settings').doc("specialDiscountOffer").get().then(async function(snapshots) {
                if (snapshots.exists) {
                    ...
                    if (specialDiscountOffer) {
                        specialDiscountOfferisEnable = specialDiscountOffer.isEnable;
                    }
                }
            }).catch(function(error) {
                console.warn('Error loading special discount offer settings:', error);
            });
            ...
            console.log('✅ [RESTAURANT EDITING] ============================================');
            console.log('✅ [RESTAURANT EDITING] Firestore ready, initializing page...');
            console.log('✅ [RESTAURANT EDITING] Reference object:', ref ? 'OK' : 'NULL');
            console.log('✅ [RESTAURANT EDITING] Reference path:', ref ? ref.path : 'NOT SET');
            console.log('✅ [RESTAURANT EDITING] Reference ID:', ref ? ref.id : 'NOT SET');
            console.log('✅ [RESTAURANT EDITING] ============================================');
            updateStatusMessage('Firestore database connection established. Ready to load restaurant data.', 'success');
            
            // Initialize immediately - no delay needed
            console.log('🔄 [RESTAURANT EDITING] Calling initRestaurantEditPage immediately...');
            initRestaurantEditPage();
        });
        
        function initRestaurantEditPage() {
            console.log('🔄 [RESTAURANT EDITING] initRestaurantEditPage called');
            console.log('🔄 [RESTAURANT EDITING] ID from PHP:', id);
            console.log('🔄 [RESTAURANT EDITING] Database available:', !!database);
            
            updateStatusMessage('Initializing restaurant edit page...', 'info');
            
            if (!database) {
                console.error('❌ [RESTAURANT EDITING] Database not available');
                updateStatusMessage('Waiting for Firestore database to be ready...', 'warning');
                // Retry after a short delay
                setTimeout(function() {
                    if (database) {
                        console.log('✅ [RESTAURANT EDITING] Database now available, retrying...');
                        updateStatusMessage('Database ready, loading restaurant data...', 'success');
                        initRestaurantEditPage();
                    } else {
                        console.error('❌ [RESTAURANT EDITING] Database still not available after retry');
                        updateStatusMessage('Error: Firestore database is not ready. Please refresh the page.', 'error');
            ...
            if (!id || id === '' || id === undefined) {
                console.error('❌ [RESTAURANT EDITING] Restaurant ID is missing or invalid:', id);
                updateStatusMessage('Error: Restaurant ID is missing. Please check the URL.', 'error');
                toastr.error('Restaurant ID is missing. Please check the URL.');
                return;
            }
            
            updateStatusMessage('Restaurant ID: ' + id + ' | Fetching data from Firestore...', 'info');
            
            // Initialize select2 (no need for document.ready here, we're already in a callback)
            ...
            console.log('🔄 [RESTAURANT EDITING] ============================================');
            console.log('🔄 [RESTAURANT EDITING] Starting data loading process...');
            console.log('🔄 [RESTAURANT EDITING] Restaurant ID:', id);
            var dbStatus = database ? 'Available' : 'NOT AVAILABLE';
            console.log('🔄 [RESTAURANT EDITING] Database instance:', dbStatus);
            var refStatus = ref ? 'Available' : 'NOT AVAILABLE';
            console.log('🔄 [RESTAURANT EDITING] Reference object:', refStatus);
            var refPath = ref ? ref.path : 'N/A';
            console.log('🔄 [RESTAURANT EDITING] Reference path:', refPath);
            console.log('🔄 [RESTAURANT EDITING] ============================================');
            
            // Validate prerequisites
            if (!database) {
                console.error('❌ [RESTAURANT EDITING] CRITICAL: Database is not initialized!');
                updateStatusMessage('Error: Database is not initialized. Please refresh the page.', 'error');
            ...
            if (!ref) {
                console.error('❌ [RESTAURANT EDITING] CRITICAL: Reference is not initialized!');
                updateStatusMessage('Error: Firestore reference is not initialized. Please refresh the page.', 'error');
                jQuery("#data-table_processing").hide();
            ...
            if (!id || id === '') {
                console.error('❌ [RESTAURANT EDITING] CRITICAL: Restaurant ID is empty!');
                updateStatusMessage('Error: Restaurant ID is missing. Please check the URL.', 'error');
                jQuery("#data-table_processing").hide();
                toastr.error('Restaurant ID is missing. Please check the URL.');
                return;
            ...
            console.log('🔄 [RESTAURANT EDITING] Attempting to fetch document from Firestore...');
            console.log('🔄 [RESTAURANT EDITING] Collection: vendors');
            console.log('🔄 [RESTAURANT EDITING] Document ID:', id);
            console.log('🔄 [RESTAURANT EDITING] Reference object type:', typeof ref);
            console.log('🔄 [RESTAURANT EDITING] Reference has get method:', typeof ref.get === 'function');
            updateStatusMessage('Fetching restaurant data from Firestore collection: vendors | Document ID: ' + id, 'info');
            
            // Ensure ref is valid before calling get()
            if (!ref || typeof ref.get !== 'function') {
                console.error('❌ [RESTAURANT EDITING] CRITICAL: Invalid reference object!');
                console.error('❌ [RESTAURANT EDITING] Reference:', ref);
                updateStatusMessage('Error: Invalid Firestore reference. Please refresh the page.', 'error');
                jQuery("#data-table_processing").hide();
                toastr.error('Invalid Firestore reference. Please refresh the page.');
                return;
            }
            
            ref.get().then(async function(docSnapshot) {
                console.log('🔄 [RESTAURANT EDITING] ============================================');
                console.log('🔄 [RESTAURANT EDITING] Firestore query completed');
                var snapshotStatus = docSnapshot ? 'Yes' : 'No';
                console.log('🔄 [RESTAURANT EDITING] Document snapshot received:', snapshotStatus);
                
                if (docSnapshot && docSnapshot.exists) {
                    updateStatusMessage('Restaurant data found in Firestore. Loading details...', 'success');
                } else {
                    updateStatusMessage('Warning: Restaurant document not found in Firestore.', 'warning');
                }
                
                try {
                    if (!docSnapshot) {
                        console.error('❌ [RESTAURANT EDITING] CRITICAL: Document snapshot is null!');
                        console.error('❌ [RESTAURANT EDITING] This means the query returned no result.');
                        console.error('❌ [RESTAURANT EDITING] Possible causes:');
                    ...
                    if (!docSnapshot.exists) {
                        console.error('❌ [RESTAURANT EDITING] CRITICAL: Document does not exist!');
                        console.error('❌ [RESTAURANT EDITING] Document ID:', id);
                        console.error('❌ [RESTAURANT EDITING] Document exists:', false);
                        console.error('❌ [RESTAURANT EDITING] This restaurant ID does not exist in Firestore.');
                        console.error('❌ [RESTAURANT EDITING] Please verify the ID in Firestore console.');
                        jQuery("#data-table_processing").hide();
                        toastr.error('Restaurant not found! Please check the restaurant ID.');
                        return false;
                    }
                    
                    console.log('✅ [RESTAURANT EDITING] Document exists: YES');
                    console.log('✅ [RESTAURANT EDITING] Document ID:', docSnapshot.id);
                    console.log('🔄 [RESTAURANT EDITING] Extracting document data...');
                    updateStatusMessage('Extracting restaurant data from document...', 'info');
                    
                    var restaurant = docSnapshot.data();
                    if (!restaurant) {
                        console.error('❌ [RESTAURANT EDITING] CRITICAL: Document data is null!');
                        updateStatusMessage('Error: Document exists but has no data.', 'error');
                        jQuery("#data-table_processing").hide();
                    ...
                    console.log('✅ [RESTAURANT EDITING] Restaurant data extracted successfully!');
                    updateStatusMessage('Restaurant data extracted. Populating form fields...', 'success');
                    console.log('📋 [RESTAURANT EDITING] ============================================');
                    console.log('📋 [RESTAURANT EDITING] RESTAURANT DATA SUMMARY:');
                    console.log('📋 [RESTAURANT EDITING] Document ID:', restaurant.id);
                    var titleValue = restaurant.title || 'N/A (MISSING!)';
                    console.log('📋 [RESTAURANT EDITING] Title:', titleValue);
                    var zoneIdValue = restaurant.zoneId || 'N/A (MISSING!)';
                    console.log('📋 [RESTAURANT EDITING] Zone ID:', zoneIdValue);
                    var phoneValue = restaurant.phonenumber || 'N/A (MISSING!)';
                    console.log('📋 [RESTAURANT EDITING] Phone:', phoneValue);
                    var locationValue = restaurant.location || 'N/A (MISSING!)';
                    console.log('📋 [RESTAURANT EDITING] Location:', locationValue);
                    var latitudeValue = restaurant.latitude || 'N/A (MISSING!)';
                    console.log('📋 [RESTAURANT EDITING] Latitude:', latitudeValue);
                    var longitudeValue = restaurant.longitude || 'N/A (MISSING!)';
                    console.log('📋 [RESTAURANT EDITING] Longitude:', longitudeValue);
                    var descriptionStatus = restaurant.description ? 'Present' : 'MISSING!';
                    console.log('📋 [RESTAURANT EDITING] Description:', descriptionStatus);
                    var countryCodeValue = restaurant.countryCode || 'N/A (MISSING!)';
                    console.log('📋 [RESTAURANT EDITING] Country Code:', countryCodeValue);
                    var photosInfo = restaurant.photos ? '(' + restaurant.photos.length + ' photos)' : '(NO PHOTOS)';
                    console.log('📋 [RESTAURANT EDITING] Has Photos:', !!restaurant.photos, photosInfo);
                    var menuPhotosInfo = restaurant.restaurantMenuPhotos ? '(' + restaurant.restaurantMenuPhotos.length + ' photos)' : '(NO PHOTOS)';
                    console.log('📋 [RESTAURANT EDITING] Has Menu Photos:', !!restaurant.restaurantMenuPhotos, menuPhotosInfo);
                    var workingHoursInfo = restaurant.workingHours ? '(' + restaurant.workingHours.length + ' days)' : '(NO WORKING HOURS)';
                    console.log('📋 [RESTAURANT EDITING] Has Working Hours:', !!restaurant.workingHours, workingHoursInfo);
                    var specialDiscountInfo = restaurant.specialDiscount ? '(' + restaurant.specialDiscount.length + ' days)' : '(NO SPECIAL DISCOUNTS)';
                    console.log('📋 [RESTAURANT EDITING] Has Special Discount:', !!restaurant.specialDiscount, specialDiscountInfo);
                    console.log('📋 [RESTAURANT EDITING] Has Admin Commission:', !!restaurant.adminCommission);
                    var categoryInfo = restaurant.categoryID ? '(' + restaurant.categoryID.length + ' categories)' : '(NO CATEGORIES)';
                    console.log('📋 [RESTAURANT EDITING] Has Category ID:', !!restaurant.categoryID, categoryInfo);
                    console.log('📋 [RESTAURANT EDITING] Has Filters:', !!restaurant.filters);
                    console.log('📋 [RESTAURANT EDITING] ============================================');
                    ...
                    if (restaurant.countryCode != "" && restaurant.countryCode != null) {
                        ...
                        console.log('✅ [RESTAURANT EDITING] Country code loaded:', restaurant.countryCode);
                    }
                    ...
                    $(".restaurant_name").val(restaurant.title);
                        console.log('✅ [RESTAURANT EDITING] Restaurant name loaded:', restaurant.title);
                    }
                    
                    // Load Address
                    if (restaurant.location) {
                    $(".restaurant_address").val(restaurant.location);
                        console.log('✅ [RESTAURANT EDITING] Address loaded:', restaurant.location);
                    }
                    ...
                    $(".restaurant_latitude").val(restaurant.latitude);
                        console.log('✅ [RESTAURANT EDITING] Latitude loaded:', restaurant.latitude);
                    }
                    
                    // Load Longitude
                    if (restaurant.longitude) {
                    $(".restaurant_longitude").val(restaurant.longitude);
                        console.log('✅ [RESTAURANT EDITING] Longitude loaded:', restaurant.longitude);
                    }
                    
                    // Load Description
                    if (restaurant.description) {
                    $(".restaurant_description").val(restaurant.description);
                        console.log('✅ [RESTAURANT EDITING] Description loaded');
                    }
                    if (restaurant.photo) {
                    ...
                    // Profile route removed for control edit new page
                    ...
                    if (restaurant.hasOwnProperty('categoryID') && restaurant.categoryID != null && restaurant.categoryID != '' && restaurant.categoryID.length > 0) {
                        ...
                        console.log('✅ [RESTAURANT EDITING] Selected categories from categoryID:', selected_category);
                        ...
                        console.log('✅ [RESTAURANT EDITING] Using cuisines from filters.Cuisine:', selected_category);
                    }
                    ...
                    await database.collection('vendor_categories').where('publish', '==', true).get().then(async function(snapshots) {
                        ...
                        console.log('✅ [RESTAURANT EDITING] Cuisines dropdown loaded with', categoriesCount, 'categories');
                    }).catch(function(error) {
                        console.error('❌ [RESTAURANT EDITING] Error loading cuisines:', error);
                    });
                    ...
                    if (restaurant.hasOwnProperty('phonenumber') && restaurant.phonenumber) {
                        $(".restaurant_phone").val(shortEditNumber(restaurant.phonenumber));
                        console.log('✅ [RESTAURANT EDITING] Phone number loaded');
                    }
                    ref_deliverycharge.get().then(async function(snapshots_charge) {
                        var deliveryChargeSettings = snapshots_charge.data();
                        try {
                    ...
                    if (restaurant.hasOwnProperty('zoneId') && restaurant.zoneId != '' && restaurant.zoneId != null) {
                        ...
                        console.log('✅ [RESTAURANT EDITING] Zone selected:', restaurant.zoneId);
                    }
                ...
                if (restaurant.hasOwnProperty('isSelfDelivery') && restaurant.isSelfDelivery != null && restaurant.isSelfDelivery != '' && restaurant.isSelfDelivery) {
                    $('#enable_self_delivery').prop('checked', true);
                        console.log('✅ [RESTAURANT EDITING] Self Delivery enabled');
                    }
                    ...
                    if (restaurant.hasOwnProperty('gst') && restaurant.gst != null && restaurant.gst != '') {
                        if ($("#gst").length) {
                            ...
                            console.log('✅ [RESTAURANT EDITING] GST loaded:', restaurant.gst);
                        }
                    }
                    ...
                    if (restaurant.hasOwnProperty('enableDND') && restaurant.enableDND) {
                        if ($("#enable_dnd").length) {
                            ...
                            console.log('✅ [RESTAURANT EDITING] DND enabled');
                        }
                    }
                    ...
                    console.log('✅ [RESTAURANT EDITING] All data loaded and displayed successfully!');
                    updateStatusMessage('All restaurant data loaded successfully! Form is ready for editing.', 'success');
                    jQuery("#data-table_processing").hide();
                    ...
                    console.error('❌ [RESTAURANT EDITING] ============================================');
                    console.error('❌ [RESTAURANT EDITING] CRITICAL ERROR in data processing!');
                    updateStatusMessage('Error: Failed to process restaurant data. ' + (error.message || 'Unknown error'), 'error');
                    var errorNameValue = error.name || 'Unknown';
                    console.error('❌ [RESTAURANT EDITING] Error Name:', errorNameValue);
                    var errorMessageValue = error.message || 'No message';
                    console.error('❌ [RESTAURANT EDITING] Error Message:', errorMessageValue);
                    var errorCodeValue = error.code || 'No code';
                    console.error('❌ [RESTAURANT EDITING] Error Code:', errorCodeValue);
                    var errorStackValue = error.stack || 'No stack trace';
                    console.error('❌ [RESTAURANT EDITING] Error Stack:', errorStackValue);
                    console.error('❌ [RESTAURANT EDITING] ============================================');
                    console.error('❌ [RESTAURANT EDITING] This error occurred while processing restaurant data.');
                    console.error('❌ [RESTAURANT EDITING] Possible causes:');
                ...
                console.error('❌ [RESTAURANT EDITING] ============================================');
                console.error('❌ [RESTAURANT EDITING] CRITICAL: Firestore query failed!');
                updateStatusMessage('Error: Firestore query failed. ' + (error.message || 'Unknown error'), 'error');
                var errorCodeValue = error.code || 'No code';
                console.error('❌ [RESTAURANT EDITING] Error Code:', errorCodeValue);
                var errorMessageValue = error.message || 'No message';
                console.error('❌ [RESTAURANT EDITING] Error Message:', errorMessageValue);
                ...
                console.error('❌ [RESTAURANT EDITING] Error Name:', errorNameValue);
                console.error('❌ [RESTAURANT EDITING] ============================================');
                console.error('❌ [RESTAURANT EDITING] This error occurred while fetching from Firestore.');
                console.error('❌ [RESTAURANT EDITING] Possible causes:');
            ...
            $(".edit-form-btn").click(async function() {
                updateStatusMessage('Validating form data before saving...', 'info');
                ...
                if (restaurantname == '') {
                    ...
                    updateStatusMessage('Preparing to save restaurant data. Uploading images and videos...', 'info');
                    await storeStoryData().then(async (resStoryVid) => {
                        updateStatusMessage('Story video processed. Processing story images...', 'info');
                        await storeImageData().then(async (IMG) => {
                            updateStatusMessage('Story images processed. Processing gallery images...', 'info');
                            await storeGalleryImageData().then(async (GalleryIMG) => {
                                updateStatusMessage('Gallery images processed. Processing menu card images...', 'info');
                                await storeMenuImageData().then(async (MenuIMG) => {
                                    geoFirestore.collection('vendors').doc(id).update({
                                        ...
                                        updateStatusMessage('Restaurant data saved successfully to Firestore!', 'success');
                                        if (resStoryVid.length > 0 || story_thumbnail != '') {
                                            if (resStoryVid.length > 0 && story_thumbnail == '') {
                                                ...
                                                database.collection('story').doc(id).set({
                                                    ...
                                                    jQuery("#data-table_processing").hide();
                                                    updateStatusMessage('All data saved successfully! Redirecting to Restaurant Control page...', 'success');
                                                    setTimeout(function() {
