        ...
        $(document).ready(function() {
            ...
            database.collection('users').where('role', '==', 'driver').orderBy('firstName').get().then(async function(snapshots) {
                snapshots.docs.forEach((listval) => {
                    ...
                    if (!data.hasOwnProperty('vendorID') || (data.hasOwnProperty('vendorID') && data.vendorID == '')) {
                        drivers.push(data);
        ...
        $(".save-form-btn").click(async function() {
            ...
            if (driverID == '') {
                ...
                if (remaining > 0) {
                    ...
                    if (sendEmailStatus) {
                        <?php if($id != ''){ ?>
                        window.location.href = "{{ route('payoutRequests.drivers.view', $id) }}";
                        <?php }else{?>
                        window.location.href = '{{ route('payoutRequests.drivers') }}';
                        <?php } ?>
                    }
                } else {
                    $(".error_top").show();
