                ...
                <a href="{{route('orders')}}?driverId={{$id}}">
                    <div class="card card-box-with-icon bg--1">
                        ...
                        <div class="card-box-with-content">
                            ...
                            <span class="box-icon ab"><img src="{{ url('images/active_restaurant.png') }}"></span>
                        </div>
                    </div>
                </a>
            </div>
                ...
                <a href="{{route('payoutRequests.drivers.view',$id)}}">
                    <div class="card card-box-with-icon bg--2">
                        ...
                        <div class="card-box-with-content">
                            ...
                            <span class="box-icon ab"><img src="{{ url('images/total_earning.png') }} "></span>
                        </div>
                    </div>
                </a>
            </div>
...
<script>
    ...
    var decimal_degits = 0;
        ...
        ref.get().then(async function(snapshots) {
            ...
            console.error('Error loading driver provider:', error);
        });
        ...
        database.collection('zone').where('publish', '==', true).orderBy('name','asc').get().then(async function (snapshots) {
            ...
            console.warn('Error loading zones (index may be needed):', error);
            ...
            database.collection('zone').where('publish', '==', true).get().then(async function (snapshots) {
                ...
                console.error('Error loading zones (fallback):', err);
            });
        });
            ...
            if (userFirstName == '') {
                ...
                storeImageData().then(IMG => {
                        database.collection('users').doc(id).update({
                            ...
                            $(".error_top").show();
                            $(".error_top").html("");
                            $(".error_top").append("<p>Error updating driver: " + error.message + "</p>");
                            window.scrollTo(0, 0);
                        });
                }).catch(err => {
                    jQuery("#data-table_processing").hide();
