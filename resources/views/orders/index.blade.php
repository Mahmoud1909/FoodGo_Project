...
@section('scripts')
    <script type="text/javascript">
        ...
        function initOrdersPage() {
            ...
            // Setup initial ref based on filters
            setupInitialRef();
            ...
            // Setup filtered records change handler
            setupFilteredRecords();
            
            // Initialize DataTable
            initOrdersDataTable();
        }
        
        function setupInitialRef() {
            if (!database || !refData) return;
            ...
            if (userID) {
                ...
                if ((order_status == 'All' || order_status != '') && search != '') {
                    ...
                    // Get all orders for user, then sort in JavaScript
                    ref = refData.where('authorID', '==', userID);
                }
            } else if (driverID) {
                ...
                if ((order_status == 'All' || order_status != '') && search != '') {
                    ...
                    // Get all orders for driver, then sort in JavaScript
                    ref = refData.where('driverID', '==', driverID);
                }
            } else if (orderStatus) {
                if (orderStatus == 'order-placed') {
                    ref = refData.where('status', '==', 'Order Placed');
                } else if (orderStatus == 'order-confirmed') {
                    ref = refData.where('status', 'in', ['Order Accepted', 'Driver Accepted']);
                } else if (orderStatus == 'order-shipped') {
                    ref = refData.where('status', 'in', ['Order Shipped', 'In Transit']);
                } else if (orderStatus == 'order-completed') {
                    ref = refData.where('status', '==', 'Order Completed');
                } else if (orderStatus == 'order-canceled') {
                    ref = refData.where('status', '==', 'Order Rejected');
                } else if (orderStatus == 'order-failed') {
                    ref = refData.where('status', '==', 'Driver Rejected');
                } else if (orderStatus == 'order-pending') {
                    ref = refData.where('status', '==', 'Driver Pending');
                } else {
                    ref = refData;
                }
            } else if (getId != '') {
                ...
                database.collection('vendors').where("id", "==", getId).get().then(async function(snapshots) {
                    ...
                    console.warn('Error loading vendor data:', error);
                });
                const getStoreName = getStoreNameFunction(getId);
                if ((order_status == 'All' || order_status != '') && search != '') {
                    ref = refData.where('vendorID', '==', getId);
                } else {
                    // Get all orders for vendor, then sort in JavaScript
                    ref = refData.where('vendorID', '==', getId);
                }
            } else {
                if ((order_status == 'All' || order_status != '') && search != '') {
                    ...
                    // Get all orders, then sort in JavaScript
                    ref = refData;
                }
            }
        }
        
        function setupFilteredRecords() {
            if (!database) return;
