                ...
                ajax: function(data, callback, settings) {
                    ...
                    ref.orderBy('createdAt', 'desc').get().then(async function(querySnapshot) {
                        ...
                        filteredRecords.sort((a, b) => {
                            ...
                            try {
                                if (orderByField === 'createdAt' || orderByField === 'subscriptionExpiryDate') {
                                    const parseDate = (val) => {
                                        if (!val) return 0;
                                        if (typeof val.toDate === 'function') return new Date(val.toDate()).getTime();
                                        if (val instanceof Date) return val.getTime();
                                        return new Date(val).getTime(); 
                                    };

                                    aValue = parseDate(a[orderByField]);
                                    bValue = parseDate(b[orderByField]);
                                }
                            } catch (err) {
